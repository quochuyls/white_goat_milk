        	...
        	<div class="navbar-main">
                <div class="menu-top-left">
                	<div class="menu-top-left-1">
                    	<img class="icon-menu-top-left-1" src="{{ URL::asset('/images/phone.png') }}"/>
                        <p class="text-menu-top-left">{{ \App\Helpers\ContactFarm::getPhone() }}</p>
                    </div>
                    <div class="menu-top-left-2">
                    	<img class="icon-menu-top-left-1" src="{{ URL::asset('/images/e_mail.png') }}"/>
                        <p class="text-menu-top-left-2">{{ \App\Helpers\ContactFarm::getEmail() }}</p>
                    </div>
                </div>
                  ...
                  <ul class="language_bar_chooser">
                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                      @if(LaravelLocalization::getCurrentLocale() != $localeCode)
                        <li>
                          <a class="lang active" rel="alternate" hreflang="{{$localeCode}}" href="{{LaravelLocalization::getLocalizedURL($localeCode) }}">{{{ $properties['native'] }}}</a>
                        </li>
                      @endif
                    @endforeach
                  </ul>
